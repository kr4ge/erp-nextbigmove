      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-100 dark:divide-border">
          <thead className="bg-slate-50 dark:bg-background-secondary">
            <tr>
              <th className="sticky left-0 z-10 whitespace-nowrap bg-slate-50 px-3 py-3 text-left text-xs font-semibold uppercase text-slate-500 dark:bg-background-secondary dark:text-slate-300 sm:px-4 lg:px-6">
                Shop
              </th>
              <th className="whitespace-nowrap px-3 py-3 text-right text-xs font-semibold uppercase text-slate-500 dark:text-slate-300 sm:px-4 lg:px-6">
                Delivered Orders
              </th>
              <th className="whitespace-nowrap px-3 py-3 text-right text-xs font-semibold uppercase text-slate-500 dark:text-slate-300 sm:px-4 lg:px-6">
                Delivered Amount
          ...
          <tbody className="divide-y divide-slate-100 bg-white dark:divide-border dark:bg-surface">
            {isLoading ? (
              ...
              rows.map((row) => (
                <tr key={row.shop} className="bg-white hover:bg-slate-50 dark:bg-surface dark:hover:bg-background-secondary">
                  <td className="sticky left-0 z-10 whitespace-nowrap bg-white px-3 py-4 text-sm text-slate-700 dark:bg-surface dark:text-slate-300 sm:px-4 lg:px-6">
                    {row.shop}
                  </td>
                  <td className="whitespace-nowrap px-3 py-4 text-right text-sm font-semibold text-slate-900 dark:text-foreground sm:px-4 lg:px-6">
          ...
          {!isLoading && totalRepurchaseRows > 0 ? (
            <tfoot className="border-t-2 border-slate-200 bg-slate-50 dark:border-border dark:bg-background-secondary">
              <tr>
                <td className="sticky left-0 z-10 whitespace-nowrap bg-slate-50 px-3 py-3 text-sm font-semibold text-foreground dark:bg-background-secondary sm:px-4 lg:px-6">
                  Total
                </td>
                <td className="whitespace-nowrap px-3 py-3 text-right text-sm font-semibold text-foreground sm:px-4 lg:px-6">
                  {formatCount(grandTotals.deliveredOrders)}
                </td>
                <td className="whitespace-nowrap px-3 py-3 text-right text-sm font-semibold text-slate-800 dark:text-slate-200 sm:px-4 lg:px-6">
