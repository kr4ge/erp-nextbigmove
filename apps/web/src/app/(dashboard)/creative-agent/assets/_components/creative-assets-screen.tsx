...
import { QC_STATUS_LABELS } from '../../video-registry/_constants/video-registry.constants';
import type { CreativeQcStatus } from '../../video-registry/_types/video-registry';
...
const QUEUE_STATUSES: CreativeQcStatus[] = ['DRAFT', 'FOR_APPROVAL', 'FOR_REVISION', 'REVISED', 'FOR_POSTING'];
...
export function CreativeAssetsScreen({ initialQuery = '', initialCreativeId, initialQcStatus, initialQueue }: {
  initialQuery?: string;
  ...
  return <div>
    ...
    {isReviewerView ? <div className="mb-3 flex flex-wrap items-center gap-2">
      <div className="flex h-9 rounded-lg border border-border/60 bg-background-secondary p-0.5" role="group" aria-label="Queue preset">
        <button type="button" onClick={() => controller.updateParams({ queue: 'REVIEW', qcStatus: '', creativeId: '' })} className={`rounded-md px-2.5 text-xs font-semibold transition ${params.queue === 'REVIEW' ? 'bg-surface text-primary shadow-sm' : 'text-muted'}`}>Review queue</button>
        <button type="button" onClick={() => controller.updateParams({ queue: '', qcStatus: '', creativeId: '' })} className={`rounded-md px-2.5 text-xs font-semibold transition ${params.queue !== 'REVIEW' ? 'bg-surface text-primary shadow-sm' : 'text-muted'}`}>All statuses</button>
      </div>
    ...
    <div className="mb-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{QUEUE_STATUSES.map((status) => <button key={status} type="button" onClick={() => controller.updateParams({ qcStatus: params.qcStatus === status ? '' : status, queue: '', creativeId: '' })} className={`card text-left transition hover:border-primary/40 ${params.qcStatus === status ? 'border-primary bg-primary-soft' : ''}`}><p className="card-label">{QC_STATUS_LABELS[status]}</p><p className="card-value mt-1">{data?.summary[status] ?? 0}</p></button>)}</div>
    <section className="panel overflow-hidden">
      <div className="flex flex-wrap items-center gap-2 border-b border-border bg-surface p-3">
        ...
        <select value={params.qcStatus} onChange={(event) => controller.updateParams({ qcStatus: event.target.value as typeof params.qcStatus, queue: '', creativeId: '' })} className={`${selectClass} w-44`}><option value="">All QC stages</option>{data?.filters.statuses.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
