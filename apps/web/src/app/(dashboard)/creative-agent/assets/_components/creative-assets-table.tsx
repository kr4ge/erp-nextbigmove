...
export function CreativeAssetsTable({ items, onReview }: { items: CreativeAsset[]; onReview: (asset: CreativeAsset) => void }) {
  return <div className="overflow-x-auto"><table className="min-w-full text-left text-sm">
    <thead className="bg-background-secondary/70"><tr>{['Preview', 'Creative', 'Creator / store', 'QC status', 'Meta link', 'Spend', 'Hook', 'Hold', 'CTR', 'Feedback', 'Submitted', ''].map((label) => <th key={label} className="whitespace-nowrap px-5 py-3 text-xs font-semibold uppercase tracking-wide text-muted">{label}</th>)}</tr></thead>
    <tbody className="bg-surface">{items.map((item) => <tr key={item.id} className="transition hover:bg-background-secondary/50 [&>td]:border-t [&>td]:border-border">
      ...
      <td className="whitespace-nowrap px-5 py-3.5 text-right font-semibold tabular-nums text-foreground">{formatCompactCurrency(item.metrics.spend)}</td>
      {/* A static has no hook or hold to measure — those read as dashes, not zeroes. */}
