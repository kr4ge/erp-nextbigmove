...
function TeamLeaderboard({ entries }: { entries: TeamLeaderboardEntry[] }) {
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-slate-100">
      <table className="min-w-[680px] w-full divide-y divide-slate-100">
        ...
        <tbody className="divide-y divide-slate-50">
          {entries.map((entry, index) => {
            ...
            return (
              <tr
                key={entry.teamCode}
                className={`align-middle ${rank <= 3 ? "bg-orange-50/20" : ""}`}
              >
                ...
                <td className="px-3 py-2">
                  ...
                  <p className="text-[11px] uppercase tracking-wide text-slate-400">
                    {entry.teamCode} · {entry.memberCount} member
                    {entry.memberCount === 1 ? "" : "s"}
                  </p>
                </td>
                <td className="px-3 py-2 text-xs tabular-nums text-slate-700">
                  {entry.onTrackCount}/{entry.totalCount}
                </td>
                <td className="px-3 py-2 text-xs tabular-nums text-slate-700">
                  {entry.avgAchievement.toFixed(1)}%
                </td>
                <td className="px-3 py-2 text-xs tabular-nums text-slate-700">
                  {entry.bestStreak}d
                </td>
