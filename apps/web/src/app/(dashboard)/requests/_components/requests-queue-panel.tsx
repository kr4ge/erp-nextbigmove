...
export function RequestsQueuePanel({
  ...
  return (
    <DashboardSection
      title="Request Queue"
      icon={<ListChecks className="panel-icon" />}
      meta={`${total} total`}
    ...
    >
        <div className="grid gap-3 lg:grid-cols-4">
          <label className="relative block">
            ...
            <input
              className="h-10 w-full rounded-xl border border-[#dce4ea] bg-surface pl-9 pr-3 text-sm text-foreground outline-none transition focus:border-[#2563eb] focus:ring-2 focus:ring-[#dbe9ff]"
          ...
          <select
            className="h-10 rounded-xl border border-[#dce4ea] bg-white px-3 text-sm text-[#12344d] outline-none transition focus:border-[#2563eb] focus:ring-2 focus:ring-[#dbe9ff] dark:border-border dark:bg-surface dark:text-foreground"
        ...
        <div className="overflow-x-auto rounded-xl border border-[#dce4ea] dark:border-border">
          <table className="w-full min-w-[720px] divide-y divide-[#e6edf5] text-sm dark:divide-border">
            <thead className="bg-[#f8fbff] text-left text-[11px] font-semibold uppercase tracking-[0.18em] text-[#73859a] dark:bg-background-secondary dark:text-slate-300">
              <tr>
            ...
            <tbody className="divide-y divide-[#edf2f7] bg-white text-[#12344d] dark:divide-border dark:bg-surface dark:text-foreground">
              {rows.length === 0 && (
                <tr>
                  <td className="px-3 py-8 text-center text-sm text-[#7b8ba1] dark:text-slate-300" colSpan={6}>
              ...
              {rows.map((row) => (
                <tr
                  key={row.id}
                  className={clsx(
                    'cursor-pointer transition hover:bg-[#f5f9ff] dark:hover:bg-background-secondary',
                    selectedBatchId === row.id ? 'bg-[#eef5ff] dark:bg-background-secondary' : '',
                  )}
                  onClick={() => onSelectBatch(row.id)}
                >
                  <td className="px-3 py-2.5">
                    <div className="font-medium">{row.sourceRequestId ?? row.id.slice(0, 8)}</div>
                    <div className="text-xs text-[#7b8ba1] dark:text-slate-300">{row.requestTitle ?? 'Untitled request'}</div>
                  </td>
                  ...
                  <td className="w-[120px] px-3 py-2.5 text-[#66788a] dark:text-slate-300">{formatShortDate(row.updatedAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex items-center justify-between">
          <p className="text-xs text-[#7b8ba1] dark:text-slate-300">
