        ...
        <tbody className="divide-y divide-[#eef2f5] bg-[#fbfcfc]">
          {batches.map((batch) => (
            <tr key={batch.id}>
              ...
              <td className="px-4 py-3 text-sm text-primary">{batch.store.name}</td>
              ...
              <td className="px-4 py-3 text-right">
                <button
                  ...
                  disabled={!canReceive}
                  className="inline-flex items-center rounded-full bg-primary px-3.5 py-2 text-[12px] font-semibold text-white transition hover:bg-[#0f3242] disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Receive
                </button>
              </td>
